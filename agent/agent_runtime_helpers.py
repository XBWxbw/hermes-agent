...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    if agent.provider == "codebuddy-acp" or str(client_kwargs.get("base_url", "")).startswith("acp://codebuddy"):
        ...
        return client
    if agent.provider == "codebuddy-http" or str(client_kwargs.get("base_url", "")).startswith("http://codebuddy-http"):
        from agent.codebuddy_http_client import CodeBuddyHTTPClient

        client = CodeBuddyHTTPClient(**client_kwargs)
        _ra().logger.info(
            "CodeBuddy HTTP client created (%s, shared=%s) %s",
            reason,
            shared,
            agent._client_log_context(),
        )
        return client
    if agent.provider == "copilot-tencent" or str(client_kwargs.get("base_url", "")).startswith("https://copilot.tencent.com"):
        from agent.copilot_tencent_client import CopilotTencentClient

        client = CopilotTencentClient(**client_kwargs)
        _ra().logger.info(
            "Copilot Tencent client created (%s, shared=%s) %s",
            reason,
            shared,
            agent._client_log_context(),
        )
